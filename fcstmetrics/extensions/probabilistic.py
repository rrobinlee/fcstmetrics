...
class ProbabilisticValidator(BaseValidator):
    ...
    def validate_prediction_intervals(self, y_true: np.ndarray, y_lower: np.ndarray, y_upper: np.ndarray, 
                                      confidence_level: float = 0.95) -> List[ValidationResult]:
        results = []
        coverage = np.mean((y_true >= y_lower) & (y_true <= y_upper))
        coverage_deviation = abs(coverage - confidence_level)
        results.append(ValidationResult(test_name="Prediction Interval Coverage",
                                        statistic=coverage,
        ...
        results.append(ValidationResult(test_name="Average Interval Width",
                                        statistic=avg_width,
        ...
        results.append(ValidationResult(test_name="Winkler Score",
        ...
        results.append(ValidationResult(test_name="Interval Sharpness",
                                        statistic=sharpness,
                                        metadata={'interpretation': 'Average interval width; smaller indicates sharper forecasts'}))
        return results
    
    def validate_quantile_forecasts(self, y_true: np.ndarray, quantile_forecasts: Dict[float, np.ndarray]) -> List[ValidationResult]:
        results = []
        for q, y_pred_q in sorted(quantile_forecasts.items()):
            ...
            results.append(ValidationResult(test_name=f"Quantile {q:.2f} Validation",
    ...
    def validate_probabilistic_calibration(self, y_true: np.ndarray, predicted_probs: np.ndarray, 
                                           n_bins: int = 10) -> List[ValidationResult]:
        results = []
    ...
    def validate_continuous_ranked_probability_score(self, y_true: np.ndarray, forecast_samples: np.ndarray) -> ValidationResult:
        ...
        return ValidationResult(test_name="Continuous Ranked Probability Score", statistic=crps, 
