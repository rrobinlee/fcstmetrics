            ...
            for col in df.columns:
                if col != 'fold':
                    ...
                    if mean_key in self.aggregate_metrics:
                        agg_row[col] = (f"{self.aggregate_metrics[mean_key]:.4f} "
...
def cross_validate_model(y: np.ndarray, y_pred_all: np.ndarray, cv: TimeSeriesCrossValidator, 
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv.split(y)):
        ...
        report = framework.run_comprehensive_validation(y_train, y_train_pred, y_test, 
                                                        y_test_pred, check_stationarity=False)
