...
class BacktestResults:
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({'timestamp': all_timestamps,
                             'actual': y_true_flat,
    ...
    def summary_stats(self) -> Dict[str, float]:
        ...
        return {'mean_error': df['error'].mean(),
...
class BacktestingFramework:
    def __init__(self, initial_window: int, step_size: int = 1, forecasting_horizon: int = 1, refit_frequency: int = 1):
        self.initial_window = initial_window
        self.step_size = step_size
        self.forecasting_horizon = forecasting_horizon
        self.refit_frequency = refit_frequency
    
    def backtest_with_predictions(self, data: pd.Series, all_predictions: np.ndarray, 
                                  model_name: str = "Backtest") -> BacktestResults:
        n = len(data)
        predictions = []
        actuals = []
        timestamps = []
        ...
        return BacktestResults(predictions=predictions, actuals=actuals, timestamps=timestamps, validation_report=report,
                               metadata={'initial_window': self.initial_window, 'step_size': self.step_size,
                                         'forecasting_horizon': self.forecasting_horizon, 'n_backtests': len(predictions)})
    
    def backtest_with_model(self, data: pd.Series, model_factory: Callable, fit_params: Optional[Dict] = None,
                            predict_params: Optional[Dict] = None) -> BacktestResults:
        ...
        return BacktestResults(predictions=predictions, actuals=actuals, timestamps=timestamps, validation_report=report,
                               metadata={'initial_window': self.initial_window, 'step_size': self.step_size,
                                         'forecasting_horizon': self.forecasting_horizon, 'refit_frequency': self.refit_frequency,
    ...
    def rolling_window_forecast_errors(self, results: BacktestResults, window_size: int = 10) -> pd.DataFrame:
        ...
        rolling_stats = pd.DataFrame({'rolling_mae': df['abs_error'].rolling(window_size).mean(),
